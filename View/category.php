...
<div style="display: flex;">
         ...
         <?php 
            ...
            if($select_publisher->rowCount()>0){
                  while($publisher=$select_publisher->fetch(PDO::FETCH_ASSOC)){?>
                        <ul style="list-style: none; padding: 0;">
                  <li>      <a href="publisher.php?publisher=
                  <?=trim($publisher['publisher']); ?>"><h6><?= $publisher['publisher']; ?></h6></a>
               </li></ul>
         <?php }
   ...
   <!-- danh sach san pham -->
<section class="products" style="width: 85%;padding-left:20px">
<h1><?=$_GET['name'];?></h1>
...
<div class="box-container">
    ...
    if (isset($_GET['id'])) {?>
      <?php
        ...
        $name_categories=$_GET['id'];//xoa khoang trang
        $get_product = $conn->prepare("SELECT b.id,b.title,b.description,b.cover,b.file,b.price,c.name
         FROM `books` as b join categories as c on b.category_id=c.id WHERE b.category_id = ?;");
        $get_product->execute([$name_categories]);
